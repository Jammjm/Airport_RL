...
class KPIs:
    ...
    def __init__(self, fleet_config):
        ...
        # ================================ 동현 수정 ===========================================
        # Flight별 시간 기록 (env 기준)
        self.flight_arrival_time = {}      # flight_id -> t_arrival (env.now)
        self.flight_gate_end_time = {}     # flight_id -> t_gate_end (env.now)


        # 절대 시간 기록 (pandas.Timestamp)
        self.flight_arrival_abs = {}       # flight_id -> ARR_TIME (Timestamp)
        self.flight_due_dep_time = {}      # 선택적으로 사용 가능
        self.flight_start_offset = {}      # flight_id -> t_start_min (env 기준 offset)

        # 각 flight에 대해 CSV의 CRS_GATE_DURATION_min (계획 gate 시간, 분 단위)
        self.flight_planned_gate_duration = {}   # flight_id -> planned gate duration [min]
        # ==================================================================================
    ...
    def report_summary(self, fleets: dict, sim_duration_min: float):
        ...
        print(f"  Turnaround Delays (based on per-flight CRS_GATE_DURATION_min):")
...
def plot_simulation_snapshot(t_now, amr_states, charger_queues):
    ...
    # AMR 유닛
    kind_colors = {'GPU': 'orange', 'FUEL': 'black', 'WATER': 'cyan', 'CLEAN': 'purple', 'CATERING': 'brown',
                   'BAGGAGE': 'magenta'}
...
def update_state(env, trigger_event: str, kpi, fleets: dict):
    ...
    # Always return snapshot for RL usage
    return snapshot
...
def _plot_gate_gantt(ts="None"):
    ...
    ax.set_title(f"Gate Occupancy Chart - Rule: {cfg.DISPATCHING_RULE}")
    ax.grid(True, axis='x', linestyle='--', alpha=0.3)
    plt.tight_layout()
    # If using timestamped output folder, file name can be simple
    # But for evaluation, ts might be '_best_reward' etc.
    if ts.startswith("EVAL") or ts.startswith("_"):
        # Custom suffix provided (e.g. from evaluate_model.py)
        # Note: ts might already contain '_' at start if passed as suffix
        # Let's handle it safely
        safe_suffix = ts if ts.startswith("_") else f"_{ts}"
        plot_path = f"{cfg.OUTPUT_DIR}/plot_gate_gantt{safe_suffix}.png"
    else:
        # Default behavior for training (ts is actual timestamp string)
        plot_path = f"{cfg.OUTPUT_DIR}/plot_gate_gantt.png"
        
    plt.savefig(plot_path, dpi=200)
    plt.close(fig)
    print(f"[VIZ] Saved gate Gantt → {plot_path}")