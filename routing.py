...
# GATE_LABELS should only include C3-C11, not chargers or corridors
GATE_LABELS = sorted([k for k in NODE_POS if k.startswith('c') and k[1:].isdigit()])
...
def _get_path_waypoints(src_label: str, dst_label: str) -> list:
    ...
    if src == DEPOT_LABEL:
        ...
        elif dst in CHARGER_LABELS:
            if dst == 'charging_1':
                return [NODE_POS['depot'], NODE_POS['depot_top_exit'], (3, 6), NODE_POS['charging_1']]
            else: # charging_2
                return [NODE_POS['depot'], NODE_POS['depot_bottom_exit'], (3, -2), NODE_POS['charging_2']]
        else:
